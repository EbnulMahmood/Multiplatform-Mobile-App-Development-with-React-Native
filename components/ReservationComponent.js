    ...
    async presentLocalNotification(date) {
     ...
     async obtainCalendarPermission() {
        ...
        if (permission.status !== 'granted') {
            ...
            if (permission.status !== 'granted') {
                Alert.alert('Permission not granted to acces the calendar');
            }
        }
        return permission;
     ...
     async addReservationToCalendar(date) {
        ...
        const endDate = new Date(Date.parse(date) + (2 * 60 * 60 * 1000)); // 2hrs
        const defaultCalendarSource = Platform.OS === 'ios' ? await Calendar.getDefaultCalendarSource(): { isLocalAccount: true, name: 'Expo Calendar'};
        let details = {
        ...
        await Calendar.createEventAsync(calendarId, {
            ...
            location: '121, Claer Water Bay Road, Clear Water Bay, Kowloonm Hong Kong'
        })
    };
